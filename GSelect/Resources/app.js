...
var steamUserName = "ImmaHacker";





// Defaults for settings.dat json
var settings = {
    "steamUserName": null,
    "appearanceSettings": {
...
var noSteamUserErrorMessage = "<li class='text errorLi'>" +
        "Looks like this is the first time you're running this program. Please add your Steam user name in General settings.<br><span onclick='openSettingsWindow()' class='errorButton'>Settings</span></li>";

var defaultFavouriteTabMessage = "<li class='text errorLi'>" +
        "You can click on Favoutire icon on top-left of each game's image and add or remove from this tab.</li>";
...
$(document).ready(function () {
    ...
    appWindow = Ti.UI.getCurrentWindow();
    //console.log("------------Available screen height:" + screen.availHeight + " and available screen width:" + screen.availWidth + "-------------------");
    //appWindow.showInspector(true);

    //appWindow.setIcon("favouriteInactive.png");
    /*var tray = Ti.UI.addTray("_converted_icon.ico", function(evt){
    ...
    /*
     //Order of stuff for the settings
     - [Ain't working]bind save settings to exit of app exit
     -readSettingsFromFile
     if settings read
     apply the read settings to current window
     else
     add the pink notifications to the tabs to give user the settings window link
     */

    try {
        console.log("------------ Applying disAreSettings to settings var ------------");
        settings = disAreSettings;
        console.log("------------ About to apply settings ------------");
        applySettings(settings);
    } catch (err) {
        //Probably the first time user is running the program so show him the error message on all the tab about setting his steam username in settings window
        $("#favouriteBlocksParent, #recentBlocksParent, #allBlocksParent, #searchBlocksParent").html(noSteamUserErrorMessage);
    }
    ...
    console.log("----- About to populate Recents tab -----");
    repopulateRecentsTab();

    //$("#recentResults").nanoScroller({
    //sliderMinHeight : 50,

    //});
    $("#recentResults").customScrollbar();


    //Sort the gameList and store it in sortedGameList variable
    /*sortedGameList = gamesList.slice();
    sortedGameList = sortByKey(sortedGameList, "name");*/
    repopulateSortedGameListTab();
    ...
    $("#favourites-tab").click(function () {
        repopulateFavouritesTab();
        changeTabContent(1);
        $("#favouriteResults").customScrollbar("remove");
        $("#favouriteResults").customScrollbar();
    });
    ...
    $("#recent-tab").click(function () {
        ...
        repopulateRecentsTab();
        changeTabContent(2);
        $("#recentResults").customScrollbar("remove");
        $("#recentResults").customScrollbar();
    ...
    $("#all-tab").click(function () {
        ...
        repopulateSortedGameListTab();
        changeTabContent(3);
        $("#allResults").customScrollbar("remove");
        $("#allResults").customScrollbar();
...
//Function to apply all the settings from the settings.dat to the UI and to initialize/reset current global variables
function applySettings(configObject) {
    console.log("applysettings running");
    /*
    
    */

    //Applying Horizontal Position
    try {
        console.log("applysettings1A");
        appWindow.setX(configObject.appearanceSettings.position.x);
    } catch (err) {
        console.log("applysettings1B");
        appWindow.setX(screen.availWidth - $("html").width());
    }

    console.log("about to apply v position");
    //Applying Vertical Position
    try {
        console.log("applysettings2A");
        appWindow.setY(configObject.appearanceSettings.position.y);
    } catch (err) {
        console.log("applysettings2B");
        appWindow.setY(0);
    }
    
    console.log("about to apply steamUserName");
    //Rework of applying steamUserName
    if (configObject.steamUserName == null || configObject.steamUserName == "") {

        console.log("steamUserName was found to be null or empty");
        steamUserName = "";
        gamesList = {};
        sortedGameList = {};
        favGamesList = [];
        ...
        /********************************************************************************
         /********************************    NOTE    ************************************
         /********************************************************************************/
        //Add what to do if there is no user name to fetch game list from
        //Also remove  '//Applying steamUserName' as we've rewowrked it right on top of this
        $("#favouriteBlocksParent, #recentBlocksParent, #allBlocksParent, #searchBlocksParent").html(noSteamUserErrorMessage);



    } else {
        console.log("steamUserName is: " + configObject.steamUserName + " in the object that was given");
        console.log("applysettings3A");
        steamUserName = configObject.steamUserName;
        console.log("applysettings3B");

        try {
            customGamesList = configObject.customGamesList;
            console.log("customGamesList was set to the one received from configObject");
        } catch (err) {
            customGamesList = [];
            console.log("settings customGamesList failed and so set to an empty array");
        }

        if (generateGameList()) {
            //Add blockCont to the container
            var c = 0;
            gamesList.forEach(function (gameItem) {
                var blockHtml = generateBlockCont(true, gameItem.appID, gameItem.gameRunLink, gameItem.name, c);
                gamesList[c].gameBlockHtml = blockHtml;
                
                c++;
            });
            
            console.log("applysettings3CA");
        } else {
            gamesList = configObject.gamesList;
            console.log("applysettings3CB");
        }
        sortedGameList = [];
        sortedGameList = gamesList.slice();
        alert(JSON.stringify(customGamesList));
        sortedGameList = sortedGameList.concat(customGamesList);
        sortedGameList = sortByKey(sortedGameList, "name");
        verbosGamesList(sortedGameList, "Coming from applySettings() line 308");

        console.log("applysettings3D");
        favGamesList = configObject.favGamesList;
        console.log("applysettings3E");
        hiddenGamesList = configObject.hiddenGamesList;
        console.log("applysettings3F");


        //console.log(JSON.stringify(sortedGameList));
        ...
        //Populate recent tab
        repopulateRecentsTab();
        console.log("applysettings3G");
        //Populate all games tab
        repopulateSortedGameListTab();
        console.log("applysettings3H");
        //Populate favourites tab
        repopulateFavouritesTab();
        console.log("applysettings3I");
    }


    generateSettingsObject();
    //Remove this (Only for seeing UI in chrome)// saveSettings();

    /*//Applying steamUserName
     if(settings.steamUserName == null || settings.steamUserName == ""){ //-------- The new settings.steamUserName was empty so don't make changes to lists and set the settings.steamUserName to what the steamUserName was during populating the tabs
     
     settings.steamUserName = steamUserName;
     return;
     
     } else if(settings.steamUserName.trim() == steamUserName.trim()){
     
     gameList = settings.gameList;
     sortedGameList = settings.gameList;
     favGamesList = settings.favGamesList;
     
     
     //Populate recents tab
     repopulateRecentsTab();
     ...
     } else{ //------- Runs if the steamUserName of the new settings.steamUserName is different than what we had previously while populating the tab with games
     ...
     favGamesList = [];
     settings.favGamesList = favGamesList;
     
     sortedGameList = gameList.slice();
     sortedGameList = sortByKey(sortedGameList, "name");
     settings.sortedGameList = sortedGameList;
...
// Function that handles the files taht were dropped into the window
function handleFileUpload(files, obj)
{
    console.log("About to loop through all the files");
    for (var i = 0; i < files.length; i++)
    {
        alert("File name: " + files[i].name + " File size:" + files[i].size);
    }
}
...
//Function to open up settings window
function openSettingsWindow() {

    generateSettingsObject();
	alert("settings.customgameslist from main app: " + JSON.stringify(settings.customGamesList));
    var dialog = Ti.UI.showDialog({url: "app://settingsWindow.html",
    ...
    dialog.addEventListener(Ti.CLOSED, function (event) {
        console.log("Settings window was closed and about to run applySettings");
        applySettings(Ti.API.get('settings'));
    });
...
//Function to repopulate recents tab
function repopulateRecentsTab() {
    //alert("GamesList lenght:" + gamesList.length);
    //console.log(JSON.stringify(gamesList));

    $("#recentBlocksParent").html("");
    
    //Adding all the gameBlocks from gamesList to the Recents tab
    gamesList.forEach(function (gameItem) {
        if (!doesArrayContainThis(hiddenGamesList, gameItem.appID)) {
            $("#recentBlocksParent").append(gameItem.gameBlockHtml);
        }
    });
    
    //Adding all the gameBlocks from customGamesList to the Recents tab
    customGamesList.forEach(function (gameItem) {
        if (!doesArrayContainThis(hiddenGamesList, gameItem.appID)) {
            $("#recentBlocksParent").append(gameItem.gameBlockHtml);
        }
    });
...
//Function to repopulate sorted game list tab
function repopulateSortedGameListTab() {
    $("#allBlocksParent").html("");
    verbosGamesList(sortedGameList, "Coming from repopulateSortedGameListTab()");
    sortedGameList.forEach(function (gameItem) {
        if (gameItem.appID != null) {
            if(gameItem.name == "xchat-2.8.9"){
                CopyToClipboard(gameItem.gameBlockHtml);
                console.log(gameItem.gameBlockHtml);
            }
...
//Function to repopulate favourites tab content
function repopulateFavouritesTab() {
    ...
    if (favGamesList.length == 0) {
        ...
        favGamesList.forEach(function (item) {
            gamesList.forEach(function (item2) {
                if (item2.appID == item) {

                    if (!doesArrayContainThis(hiddenGamesList, item2.appID)) {
...
//Function to Generate settings object for the settings window or to save settings in a file
function generateSettingsObject() {
    ...
    settings.appearanceSettings.resolution.w = appWindow.getBounds().width;
    console.log("A");
    //Remove this (Only for seeing UI in chrome)// 
    settings.appearanceSettings.resolution.h = appWindow.getBounds().height;
    console.log("A");
    //Remove this (Only for seeing UI in chrome)// 
    settings.appearanceSettings.position.x = appWindow.getBounds().x;
    console.log("A");
    //Remove this (Only for seeing UI in chrome)// 
    settings.appearanceSettings.position.y = appWindow.getBounds().y;
    settings.favGamesList = favGamesList;
    settings.hiddenGamesList = hiddenGamesList;
    settings.customGamesList = customGamesList;
    settings.gamesList = gamesList;
    console.log("A");
...
function tempFunctionToSaveGameIndex() {
    var file = Ti.Filesystem.getFileStream(Ti.App.getHome() + "\\Resources\\Settings\\", "tempGameIndex.js");
    file.open(Ti.Filesystem.MODE_WRITE);
    file.write(JSON.stringify({"sortedGameList": sortedGameList, "gamesList": gamesList, "games": games}));
    file.close();
}


//Function to save all the preferences to the settings.dat file
function saveSettings() {
    ...
    console.log("------ savesettings B");
    var file = Ti.Filesystem.getFileStream(Ti.App.getHome() + "\\Resources\\Settings\\", "settings.js");
    file.open(Ti.Filesystem.MODE_WRITE);
    file.write("var disAreSettings = " + JSON.stringify(settings));
    file.close();
...
function searchForThis(caller) {
    ...
    if (searchQuery.trim().length == 0) {
        $("#searchResults").customScrollbar("remove");
        $("#searchResults").customScrollbar();
        return;
    }
    ...
    if (errorOccured || searchResult.length == 0) {
        $("#searchBlocksParent").append("<li class='text errorLi'>" + errorMessage + "</li>");
    }
    else {

        searchResult.forEach(function (gameItem) {
            console.log("Result from sortedGamesList Id:" + (gameItem - 1));
            $("#searchBlocksParent").append(generateBlockCont(true/*doesFileExists(gameImagesPath, sortedGameList[gameItem - 1].appID + ".jpg")*/, sortedGameList[gameItem - 1].appID, sortedGameList[gameItem - 1].gameRunLink, sortedGameList[gameItem - 1].name, gameItem - 1));
            //$("#searchBlocksParent").append(sortedGameList[gameItem - 1].gameBlockHtml);
...
//Function for debuging. Copys a string to clipboard
function CopyToClipboard(text) {
    //Ti.UI.Clipboard.setData('text/plain', text);
}




//Function for debuging. Shows the entire list of game and their index place.
function verbosGamesList(list, message){
    if(message !== null){
        console.log(message);
    }
    
    for(var count = 0; count < list.length; count++){
        console.log(count + ". " + list[count].name + " [ID: " + list[count].appID + "]");
    }
}
...
function generateGameList() {
    var steamAPILink = "https://query.yahooapis.com/v1/public/yql?q=select+%2A+from+xml+where+url%3D%22http%3A%2F%2Fsteamcommunity.com%2Fid%2F" + steamUserName + "%2Fgames%3Ftab%3Dall%26xml%3D1%22&format=json&diagnostics=false";
    var myData;
    console.log("--About to ajax from steam API--");
    ...
    try {
        $.ajax({
            ...
            success: function (data) {
                console.log("--Data received for generateGameList function.--");
                var gamesListTemp = data.query.results.gamesList.games.game;
                gameList2 = data;
                gamesList = new Array();
                gamesListTemp.forEach(function (gameItem) {


                    /*var gameItem = gamesListTemp[count - 1];
                    ...
                    //Mark the game to auto hide if 'settings.appearanceSettings.showObsoleteGames' is set to false.
                    if (gameItem.name.indexOf("(Obsolete)") != -1) {
                        hiddenGamesList.push(gameItem.appID);
                    }
                ...
                console.log("reached the end of success");
            },
        ...
        console.log("problem ajaxing data: " + err.message);
    }
    return readFromSteamAPI;
...
function generateBlockCont(imageExists, gameId, gameRunLink, gameName, c) {
    ...
    var temp = "<li class='gameImageLi pixelated' style='background-image: url(" + imageName + ");'>" +
            "<div class='gameImageOverlay'>" +
            "<div onclick='favContClicked(" + gameId + ", this)' class='favCont'><img src='";
    ...
    temp += "' class='favImg'></div>" +
            "<div class='gameNameDiv text pixelated'>" + c + ". " + gameName + "</div></div>" +
            "</li>";
    return temp;
}
...
//Function to reIndex all the games in sortedGamesList
...
function doesFileExists(pathLoc, fileName) {
    var disFile = Ti.Filesystem.getFile(pathLoc, fileName);
...
//Function to read the file using path and file name
function readFile(pathLoc, fileName) {
    if (doesFileExists(pathLoc, fileName)) {
        var file = Ti.Filesystem.getFile(pathLoc, fileName);
        return file.read();
    }
    return false;
}
...
function changeTabContent(tabNum) {
    ...
    if (tabNum == 1) {
        ...
        $("#favouriteResults").css("height", "654px");//500px");//This value comes from .nano's styling in your mainStyleSheet.css
        ...
        $("#recentResults").css("height", "654px");//500px");//This value comes from .nano's styling in your mainStyleSheet.css
        ...
        $("#allResults").css("height", "654px");//500px");//This value comes from .nano's styling in your mainStyleSheet.css
        ...
        $("#searchResults").css("height", "654px");//500px");
