...
var noSteamUserErrorMessage = "<li class='text errorLi'>" +
		"Looks like this is the first time you're running this program. Please add your Steam user name in General settings. <br><span onclick='openSettingsWindow()' style='position:relative; top: 10px; background-color: #69d2e7; color: #000; padding: 4px 6px; border-radius: 3px; box-shadow: 2px 2px 1px 1px #000;'>Settings</span></li>";
...
$(document).ready(function () {
	...
	-readSettingsFromFile
		if settings read
			...
			add the pink notifications to the tabs to give user the settigns window link
	...
	*/
	...
	var readData = readFile(Ti.App.getHome(), "settings.dat");
	alert(Ti.App.getHome() + " settings.dat" );
	if(readData){
		alert(readData);
		console.log("-----Settings.dat file was read -----");
		//alert(settings);
		console.log("======A==========");
		console.log("About to parse JSON");
		try{
			alert(readData.steamUserName);
		}catch(err){
			alert("Error while outputting readData.steamName" + err.message);
		}
		
		
		try{
			settings = Ti.JSON.parse(readData);
			alert("This shits fram settings.dat: " + readData.steamUserName);
		}catch(err){
			//alert(settings.error.message);
			alert("Error while parsing settings.dat " + err.message);
		}
		
		
		alert("Dis da steam usah name" + settings.steamUserName);
		console.log("JSON parsing for the read settings failed");
		applySettings(settings);
		console.log("======A==========");
		
	}else{
		console.log("======B==========");
		console.log("-----Failed to read settings.dat file -----");
		console.log("======B==========");
	}
...
function applySettings(configObject) {
	...
	if(configObject.steamUserName == null || configObject.steamUserName == ""){
		...
		console.log("applysettings3C");
		sortedGameList = configObject.sortedGameList;
...
function saveSettings() {
    ...
    //alert(t);
	alert("----- saveSettings function called -----");
	
	settings.gamesList.forEach(function(gameItem){
		gameItem.gameBlockHtml = null;
	});
	
	alert("------ sasvesettings B");
    var file = Ti.Filesystem.getFileStream(Ti.App.getHome(), "tempsettings2.dat");
    file.open(Ti.Filesystem.MODE_WRITE);
    file.write(JSON.stringify(settings));
    file.close();
	alert("----- settings file saved -----");
}
...
function searchForThis(caller) {
    ...
    if (errorOccured || searchResult.length == 0) {
        ...
        searchResult.forEach(function (gameItem) {
            //$("#searchBlocksParent").append((true, gameList[gameItem - 1].appID, gameList[gameItem - 1].gameRunLink, gameList[gameItem - 1].name, gameItem - 1));
            $("#searchBlocksParent").append(gamesList[gameItem - 1].gameBlockHtml);
        });
